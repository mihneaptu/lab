...
/* Everything below is scoped to this function rather than left on the
   page's shared global scope. These are classic scripts: every top-level
   const in an exhibit lands in the same namespace as scripts/theme.js and
   scripts/haptics.js, and a name used twice is not a shadow but a
   SyntaxError that kills whichever file loads second. This exhibit's
   script loads FIRST, so the casualty would be theme.js, and the symptom
   would be a theme button that silently stopped working on one page. */
(() => {
  const root = document.documentElement;
  const toggle = document.querySelector(".sky-toggle");
  const label = document.querySelector(".sky-label");

  /* Storage can be unavailable, not merely empty: a locked-down profile
     or a blocked third-party context throws on access rather than handing
     back null. Remembering the sky is a nice-to-have, so swallow the
     refusal and let the document keep whatever it already wears.

     This matters more here than anywhere else in the lab. Unguarded, the
     click handler below dies on its first line — and this exhibit IS the
     switch, with no corner toggle to fall back on, so the page would be
     left with no way to change the theme at all. */
  ...
  toggle.addEventListener("click", () => {
    /* localStorage, not root.dataset, is the source of truth for
       ...
       cancel itself out. */
    const next =
      (readSavedTheme() || root.dataset.theme) === "dark"
        ? "light" : "dark";
    saveTheme(next);
  ...
  function prepareSkyForSuspension() {
    clearTimeout(labelTimer);
    cancelAnimationFrame(flipFrame);
    root.dataset.theme = readSavedTheme() || root.dataset.theme;
  ...
  /* Park the loop and put every part back where it belongs, including the
     residue on screen: zeroing the physics while a transform is still
     written leaves a part sitting visibly off-centre forever. */
  function settleParts() {
    ...
    for (const part of parts) {
      ...
      if (part.el.style.transform) part.el.style.transform = "";
    }
  }
  ...
  /* wake() guards the way in; this guards the way out. Turning the
     preference on mid-visit should stop what is already swinging, rather
     than letting the springs ring down at their own pace first. */
  calm.addEventListener("change", () => {
    if (calm.matches) settleParts();
  });
